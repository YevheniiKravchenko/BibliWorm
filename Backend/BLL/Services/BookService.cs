...
public class BookService : IBookService
{
    ...
    public BookModel GetBookById(Guid bookId)
    {
        ...
        if (bookId == Guid.Empty)
        {
            ...
            bookModel.PublicationDate = null;
        } 
        else
        { 
            var book = _unitOfWork.Books.Value.GetById(bookId);
            bookModel = _mapper.Value.Map<BookModel>(book);
        }
    ...
    public List<BookingListItemModel> GetUserBookings(int userId)
    {
        var userBookings = _unitOfWork.Bookings.Value.GetAll()
            .Where(b => b.UserId == userId);
        var userBookingsModels = _mapper.Value.Map<List<BookingListItemModel>>(userBookings);

        return userBookingsModels;
    }
